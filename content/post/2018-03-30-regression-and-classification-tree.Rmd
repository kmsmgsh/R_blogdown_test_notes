...
The every cross lane in each axes is a partition of the sample space.

So the candidate partition is the every point in horse power axis or the whellbase axis(as the figure example). 
This structure is not that like a tree structure, but actually it's equivelant. We can use "Questions" structures to transfer these two structure.

For example, the basic format of questions is "Is $x\in A$?", to the partition of the picture is "Is Horse power great than 0.6?", then a candidate question can lead a candidate split. Anser "Yes" to $t_L$ and all $x_n$ in t answer "No" to $t_R$.
